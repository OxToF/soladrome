...
// The native-SOL reserve and the failure decoder live in their own dependency-free module,
// re-exported here so every call site keeps a single import.
export { SOL_FEE_RESERVE, spendableSol } from "./txerror";
...
/// Explain a failed transaction. Thin adapter over `txerror.ts`, which holds the logic and
/// stays free of the IDL import so it can be unit-tested under `node --test`.
export function explainTxError(err: unknown, ixs?: TransactionInstruction[]): string {
  return explainInstructionError(
    err,
    ixs?.map((i) => i.programId.toBase58()),
    (idl as { errors?: IdlErrorEntry[] }).errors,
  );
}
