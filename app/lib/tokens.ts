...
// ── Devnet Token-2022 stock fixtures ─────────────────────────────────────────
//
// Generated by `scripts/mint_devnet_xstocks.ts`, which is the file to edit. They exist so the
// Token-2022 admission path is observable in the UI instead of only in the bankrun suite:
// `admitted: false` entries are in the picker ON PURPOSE, so that selecting one and pressing
// Create Pool shows the program refusing it.
//
// ☢️ Their mint authorities are derived from a public string, so anyone can mint them without
// limit. They are gated on the cluster below and must never reach a mainnet build.
export interface DevnetMockToken extends TokenInfo {
  admitted: boolean;
  profile:  string;
  why:      string;
}

// `as unknown as` and not a plain assertion: the checked-in placeholder has an empty `tokens`
// array, which TypeScript infers as `never[]`, and `never[]` does not overlap the real shape.
const DEVNET_XSTOCK_SOURCE = devnetXStocks as unknown as { tokens: DevnetMockToken[] };

// NEXT_PUBLIC_ vars are inlined at build time, so this resolves to a constant in the bundle.
// Helius spells its endpoints `devnet.helius-rpc.com` / `mainnet.helius-rpc.com`, and the
...
// Mainnet launch pool tokens — classic SPL only.
//
// ⚠️ This comment used to read "the AMM's `Account<Mint>` / `Program<Token>` reject Token-2022,
// so USDG/CASH/PYUSD and the xStocks are intentionally NOT here: they cannot be pooled without
// an AMM upgrade". That upgrade landed on 2026-09-01: `create_pool` takes two token programs
// and `token_ext::require_supported_mint` decides admission, so the program constraint is gone.
// Their absence from this list is now a registry decision and nothing more, and the devnet
// fixtures above are exactly the Token-2022 pairs the AMM does accept. Anything added here
// still has to pass `scripts/check_token_registry.ts`, which asserts classic SPL for this list.
//
...
export const TRUSTED_MINTS = new Set([
  ...
  // ── Devnet Token-2022 fixtures (empty on any non-devnet build) ──
  ...DEVNET_MOCK_TOKENS.map((t) => t.mint),
]);
