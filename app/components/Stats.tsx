...
import { toUi, floorVault, marketVault } from "@/lib/program";
...
export function Stats() {
  ...
  const stats = useMemo(() => {
    ...
    // SOLA is quoted off the bonding curve alone. There is no SOLA/USDC AMM pool
    // (and none is planned — see the launch pool list), so the former AMM quote
    // read "No pool" permanently and the spread it displayed was meaningless.
    // oSOLA is an option struck at the $1 floor, so its worth is the curve price
    // less the strike, floored at 0 — it is 0 for as long as SOLA sits at 1.
    const osolaIntrinsic = Math.max(0, curvePrice - 1);
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 sm:gap-4 mb-6 sm:mb-8">
      ...
      <div className="card p-3 sm:p-5 text-center">
        ...
        <p className="font-bold text-brand-green text-base sm:text-lg leading-tight truncate">
          {fmt4(stats.osolaIntrinsic)} USDC
        </p>
        <p className="text-[10px] text-gray-600 mt-0.5">exercise value</p>
        <p className="text-[10px] text-gray-600 mt-1 truncate">curve − 1 USDC strike</p>
      </div>
      <div className="card p-3 sm:p-5 text-center">
